...
class BertAttentionMask(Layer):
    def __init__(self, **kwargs):
        super(BertAttentionMask, self).__init__(**kwargs)

    def call(self, inputs, mask=None):
        batch_size, length = inputs.get_shape()
        mask = K.expand_dims(inputs, axis=1)
        mask = K.repeat_elements(mask, length, axis=1)
        mask = K.expand_dims(mask, axis=1)
        return mask

    def compute_output_shape(self, input_shape):
        return (input_shape[0], 1, input_shape[-1], input_shape[-1])
