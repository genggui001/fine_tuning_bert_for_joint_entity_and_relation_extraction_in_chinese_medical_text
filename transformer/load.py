...
def load_google_bert(base_location: str = './google_bert/downloads/multilingual_L-12_H-768_A-12/',
                     use_attn_mask: bool = True, max_len: int = 512, verbose: bool = False, bert_attention_mask = True, use_diff_attention_mask = False, add_unsed_token = False) -> keras.Model:
    ...
    if add_unsed_token:
        create_vocab_size = vocab_size + TextEncoder.BERT_UNUSED_COUNT
    else:
        create_vocab_size = vocab_size
       
    model = create_transformer(embedding_layer_norm=True, neg_inf=-10000.0, use_attn_mask=use_attn_mask,
                               vocab_size=create_vocab_size, accurate_gelu=True, layer_norm_epsilon=1e-12, max_len=max_len,
                               ...
                               use_diff_attention_mask = use_diff_attention_mask)
    ...
    for var_name, _ in var_names:
        ...
        if w_id is not None and qkv is None:
            ...
            if w_id == 1:  # pos embedding
                ...
                if add_unsed_token:
                    vocab_size -= TextEncoder.BERT_UNUSED_COUNT
                # ours: unk, [vocab], pad, msk(mask), bos(cls), del(use sep again), eos(sep)(, 99 unused)
                ...
                if add_unsed_token:
                    weights[w_id][vocab_size + TextEncoder.SPECIAL_COUNT : vocab_size + TextEncoder.SPECIAL_COUNT + TextEncoder.BERT_UNUSED_COUNT] = saved[1: 1 + TextEncoder.BERT_UNUSED_COUNT]
            else:
...
def load_baidu_bert(base_location: str = './ERNIE_stable/tf_params/',
                     use_attn_mask: bool = True, max_len: int = 512, verbose: bool = False, bert_attention_mask = True, use_diff_attention_mask = False) -> keras.Model:
    bert_config = BertConfig.from_json_file(base_location + 'ernie_config.json')
    init_checkpoint = base_location + 'model.ckpt'
    var_names = tf.train.list_variables(init_checkpoint)
    check_point = tf.train.load_checkpoint(init_checkpoint)
    # 4个特殊字符，不含[UNK]
    vocab_size = bert_config.vocab_size - 4
    model = create_transformer(embedding_layer_norm=True, neg_inf=-10000.0, use_attn_mask=use_attn_mask,
    ...
    for var_name, _ in var_names:
        ...
        if w_id is not None and qkv is None:
            ...
            if w_id == 1:  # pos embedding
                ...
                # theirs: PAD, CLS, SEP, MASK, [vocab], UNK
                saved = check_point.get_tensor(var_name)  # vocab_size, emb_size
                # weights[our_position] = saved[their_position]
                weights[w_id][0] = saved[bert_config.vocab_size - 1]  # unk 最后一个
                weights[w_id][1:vocab_size] = saved[4:bert_config.vocab_size - 1]
                weights[w_id][vocab_size + TextEncoder.PAD_OFFSET] = saved[0]
                weights[w_id][vocab_size + TextEncoder.MSK_OFFSET] = saved[3]
                weights[w_id][vocab_size + TextEncoder.BOS_OFFSET] = saved[1]
                weights[w_id][vocab_size + TextEncoder.DEL_OFFSET] = saved[2]
                weights[w_id][vocab_size + TextEncoder.EOS_OFFSET] = saved[2]
            else:
                weights[w_id][:] = check_point.get_tensor(var_name) if not unsqueeze else \
                    check_point.get_tensor(var_name)[
                        None, ...]
