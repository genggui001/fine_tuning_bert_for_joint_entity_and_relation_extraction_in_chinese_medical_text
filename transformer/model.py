...
from transformer.layers import MultiHeadAttention, Gelu, LayerNormalization, BertAttentionMask
...
def create_transformer(embedding_dim: int = 768, embedding_dropout: float = 0.1, vocab_size: int = 30000,
                       ...
                       accurate_gelu: bool = False, bert_attention_mask = False, use_diff_attention_mask = False, 
                       type_vocab_size=TextEncoder.NUM_SEGMENTS ) -> keras.Model:
    ...
    if use_diff_attention_mask:
        for i in range(num_layers):
            if bert_attention_mask is False:
                attn_mask = Input(batch_shape=(None, 1, max_len, max_len), name='attention_mask_input_{}'.format(i),
                                  dtype=K.floatx()) if use_attn_mask else None
                mask_inputs.append(attn_mask)
            else:
                bert_attention_mask_input = Input(shape=(max_len,), name="attention_mask_input_{}".format(i), dtype=K.floatx())
                attn_mask = BertAttentionMask(name='bert_attention_mask_{}'.format(i))(bert_attention_mask_input)
                mask_inputs.append(bert_attention_mask_input)
            
            mask_outputs.append(attn_mask)
        ...
        if bert_attention_mask is False:
            ...
            bert_attention_mask_input = Input(shape=(max_len,), name="attention_mask_input", dtype=K.floatx())
            attn_mask = BertAttentionMask(name='bert_attention_mask')(bert_attention_mask_input)
            mask_inputs = [bert_attention_mask_input]
        
        mask_outputs = [attn_mask] * num_layers
    ...
    embedding_layer = Embedding(embedding_dim, embedding_dropout, vocab_size, max_len, trainable_pos_embedding,
                                use_one_embedding_dropout, embedding_layer_norm, layer_norm_epsilon, type_vocab_size)
    x = embedding_layer(inputs)
    for i in range(num_layers):
        x = EncoderLayer(embedding_dim, num_heads, d_hid, residual_dropout,
                         attention_dropout, use_attn_mask, i, neg_inf, layer_norm_epsilon, accurate_gelu)(x, mask_outputs[i])

    return keras.Model(inputs=inputs + mask_inputs, outputs=[x], name='Transformer')
